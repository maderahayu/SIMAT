            ...
            <div class="container-fluid">
                ...
                <div class="card pb-1 mb-2">
                    <div class="card-body pt-4 row">
                        ...
                        <form action="{{ route('logbookUpdate', $lg->logbookId) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            
                            <div class="form-group">
                                <label>Tugas</label>
                                <input type="text" name="tugasId" value="{{ $lg->task->judul }}" class="form-control" readonly>
                            </div>

                            <div class="form-group">
                                <label>Tanggal</label>
                                <input type="date" name="tglLogbook" value="{{ $lg->tglLogbook }}" class="form-control" required>
                            </div>

                            <div class="form-group">
                                <label for="deskripsi">Deskripsi</label>
                                <textarea name="deskripsi" class="form-control" placeholder="Contoh: Membuat feed instagram bertemakan kemerdekaan untuk menyambut 17 agustus" required>{{ $lg->deskripsi }}</textarea>
                            </div>

                            <div class="form-group">
                                <label>Status Tugas</label>
                                <select name="status" class="form-control" required>
                                    <option style="display:none" value="{{$lg->task->status}}">{{$lg->task->status}}</option>
